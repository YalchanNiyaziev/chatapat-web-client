...
const useChatConversationHistory = () => {
        ...
        const fetchSidebarDataBySelectedItemType = useCallback(() => {
                            ...
                            const foundSearchResults = res.data.map(result => (
                                {
                                    ...
                                    isSenderConnectionRequest: result.isSenderConnectionRequest,
                                }
                            ));
    ...
    const sendUserConnectionRequest = requestedUsername => {

        const sendConnectionRequest = requestedUser => {
            const requestedUseInfo = {
                username: requestedUser,
            }
            console.log("Requested USER", requestedUser);
            api.doSendConnectionRequest(autService.getUsername(), requestedUseInfo)
                .then(res => {
        ...
        const conformProps = {
            message: `Are you sure to send connection request to  user with username ${requestedUsername}?`,
            header: 'Confirm send user connection request',
            icon: 'pi pi-exclamation-triangle',
            accept: () => sendConnectionRequest(requestedUsername),
            reject: () => {
            },
        };
    ...
    const acceptUserConnectionRequest = acceptedUsername => {
        const acceptConnection = acceptedUser => {
            ...
            api.doAcceptConnectionRequest(autService.getUsername(), acceptedUser)
                .then(res => {
                    if (visibleConnectionDialog) {
                        const search = {
                            searchByUsername: watch('searchByUsername'),
        ...
        const conformProps = {
            message: `Are you sure to accept connection request from user with username ${acceptedUsername}?`,
        ...
        const removeUserConnection = removedUsername => {

            const removeUser = removedUser => {
                console.log("Removed USER", removedUser);
                api.doRemoveConnection(autService.getUsername(), removedUser)
                    .then(res => {
                        if (visibleConnectionDialog) {
                            const search = {
                                searchByUsername: watch('searchByUsername'),
            ...
            const conformProps = {
                message: `Are you sure to remove user with username ${removedUsername}?`,
                header: 'Confirm remove user connection',
                icon: 'pi pi-exclamation-triangle',
                accept: () => removeUser(removedUsername),
                reject: () => {
                },
            };
        ...
        const blockUserConnection = blockedUsername => {

            const blockUser = blockedUser => {
                console.log("BLOCKED USER", blockedUser);
                api.doBlockConnection(autService.getUsername(), blockedUser)
                    .then(res => {
                        if (visibleConnectionDialog) {
                            const search = {
                                searchByUsername: watch('searchByUsername'),
            ...
            const conformProps = {
                message: `Are you sure to block user with username ${blockedUsername}?`,
                header: 'Confirm bLock user connection',
                icon: 'pi pi-exclamation-triangle',
                accept: () => blockUser(blockedUsername),
                reject: () => {
                },
            };
        ...
        const unblockUserConnection = unblockedUsername => {
            const unblockUser = unblockedUser => {
                console.log("UNBLOCKED USER", unblockedUser);
                api.doUnblockConnection(autService.getUsername(), unblockedUser)
                    .then(res => {
                        if (visibleConnectionDialog) {
                            const search = {
                                searchByUsername: watch('searchByUsername'),
            ...
            const conformProps = {
                message: `Are you sure to unblock user with username ${unblockedUsername}?`,
                header: 'Confirm unbLock user connection',
                icon: 'pi pi-exclamation-triangle',
                accept: () => unblockUser(unblockedUsername),
                reject: () => {
                },
            };
            confirm(conformProps);
